"""
╔══════════════════════════════════════════════════════════════════╗
║        BGYO: ACES OF P-POP — THE LIGHT STAGE  v15.0            ║
║        game_objects.py  — Notes, Particles, Spotlights          ║
╚══════════════════════════════════════════════════════════════════╝

PURPOSE & ROLE IN THE ARCHITECTURE
────────────────────────────────────
This module defines every visual game object that lives on the canvas
during gameplay and on the home/title screen.  It has no knowledge of
tkinter widgets, screen layout, or game state — it only owns the data
and per-frame physics of individual objects.

All rendering (calling cv.create_*) is performed either inside the
object's own draw() method (Spotlight) or by bgyo_game.py's render
loop which reads each object's x/y/col/life fields directly.

Dependency flow (game_objects is a near-leaf — nothing imports it
except the game layer):

    constants.py  ←  game_objects.py  ←  bgyo_game.py

Imports from constants.py:
    MEMBER_COLORS     — spotlight beam colours cycle through these
    BG_COL            — base colour for additive spotlight blending
    dim, blend,       — colour math utilities used in Spotlight.draw()
    additive_blend,
    spotlight_col
    hex_to_rgb        — RGB channel access for per-ray colour computation
    W, H              — default canvas dimensions (Spotlight floor geometry)

OBJECT LIFECYCLE SUMMARY
─────────────────────────
  Note        — created by bgyo_game._update_game() when a spawn_t is
                reached; removed when it passes the hit zone or is hit.
  Particle    — created in bursts on PERFECT/GOOD hits and by SideEffect;
                self-destructs when life <= 0.
  Spark       — created on hit events; faster decay than Particle.
  Flash       — floating score text (+100 PERFECT etc.); rises and fades.
  SideEffect  — manages a burst of Particles plus a label for the left/
                right side panels; short-lived, self-destructs.
  Spotlight   — persistent concert lights; one instance per beam (8 total),
