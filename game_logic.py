...
║   game_logic.py  ·  Note spawning, hit detection, scoring       ║
╚══════════════════════════════════════════════════════════════════╝

PURPOSE & ROLE IN THE ARCHITECTURE
────────────────────────────────────
This module contains the pure gameplay mechanics that were previously
embedded inside BGYOGame in bgyo_game.py.  Extracting them here
achieves a clean separation of concerns:

    game_logic.py  — WHAT happens (rules, scoring, physics)
    game_renderer.py — HOW it looks (drawing calls)
    bgyo_game.py   — WHEN it happens (game loop, screen transitions)

All functions are stateless helpers that operate on the `gs` (game
state) dict passed in by reference.  They do not import tkinter,
pygame, or settings_state — they only read constants and create/
mutate game_objects.

Dependency chain:

    constants.py
    game_objects.py  ←  game_logic.py  ←  bgyo_game.py

THE GAME STATE DICT  (`gs`)
─────────────────────────────
`gs` is created by BGYOGame._start_game() and is the single mutable
container for all live session data.  Key groups:

    Scoring:   score, combo, max_combo, perfect, good, miss
    Timing:    elapsed, countdown, countdown_go, note_timer
               song_wall_start, song_duration
    Notes:     notes (list[Note]), beat_chart, chart_cursor, beat_mode
    Tuning:    speed, hit_win, perf_win, ival_min, ival_max
    State:     paused, ended, loading, advancing, menu_paused
    Song:      song_names, song_paths, song_idx, current_mp3, endless
    Visual:    combo_flash, member_idx, member_timer, num_lanes

SCORING MODEL
──────────────
    PERFECT hit  → +300 × (1 + combo ÷ 10)  points
                   Combo increment, max_combo update
    GOOD hit     → +100 points, combo increment
    MISS (key)   → -50 points (floor 0), combo reset
    MISS (scroll)→ -0 points, combo reset
                   (Note scrolls past hit bar without input)

TIMING WINDOWS  (from DIFFICULTY preset in constants.py)
──────────────────────────────────────────────────────────
    perf_win : half-width of PERFECT window (depth units)
               |note.depth - HIT_DEPTH| ≤ perf_win → PERFECT
    hit_win  : half-width of GOOD window (depth units)
               perf_win < |note.depth - HIT_DEPTH| ≤ hit_win → GOOD
               |note.depth - HIT_DEPTH| > hit_win → miss (no note found)

NOTE DEPTH MODEL
─────────────────
In beat-chart mode:
    note.depth = HIT_DEPTH − (note.beat_t − audio.position()) × speed

    The note's depth is recalculated every frame from the live audio
    position.  This keeps the note visually "locked" to the music
    even if the frame rate varies.  A note is exactly at HIT_DEPTH
    when audio.position() == note.beat_t.

In random (fallback) mode:
    note.depth += speed × dt

    Simple linear advance without audio sync.

GRADE THRESHOLDS
─────────────────
    S  — accuracy ≥ 95% AND perfect count > 15
    A  — accuracy ≥ 85%
    B  — accuracy ≥ 70%
    C  — accuracy ≥ 50%
    D  — accuracy < 50%
