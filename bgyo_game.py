...
SETUP (one-time):
  pip install pygame librosa pillow

FOLDER STRUCTURE:
  bgyo_game.py
  songs/          ← MP3s here (filenames must match song titles)
  songs/preview/  ← Chorus preview MP3s (format: "SongName - Chorus.MP3")
  images/
    bgyo_logo.png
    bgyo_members_grouped.png
...
v18.3 ENHANCEMENTS (User-Requested)
  ...
  • bgyo_during_game photo now drawn BELOW the HUD song-title box, not behind it
  • Banner is always fully visible during gameplay — no longer hidden behind the panel
  • Image thumbnail height reduced slightly (0.14 vs 0.18) so it fits neatly below the box
  • New _draw_game_banner_below_hud() method handles correct Y-positioning

  ACES TRIVIA Introduction Screen Redesigned
  • Title "✦ BGYO ACES TRIVIA ✦" now animates with color-cycling through member colors
  • Glowing pulse effect on title before game starts for an exciting intro feel
  • Stat badges (Questions · Time · Topic) centered and aligned with the card
  • All instruction text centered, properly sized (12–13 px), wraps to card width
  • Card is wider (560 px) for better text alignment — no cramped layout
  • "BACK TO MAIN STAGE" replaces "EXIT GAME" for clearer navigation

  Rankings — Logged-in Player Row Highlighted
  • Logged-in player's rows are gold-tinted (#1a1200 bg) with a gold border
  • Player name prefixed with ★ and shown in bold for instant identification
  • Personal best summary bar below the leaderboard table (score, grade, acc, combo, song)
  • Summary shows only when a registered account is logged in

  Trivia Score Saving — Account-Restricted
  • Logged-in users: name field is READ-ONLY (locked to account username)
  • Prevents leaderboard impersonation — only the account name is accepted
  • Guest accounts: save option is hidden; info message prompts registration
  • Guests still see their score and grade — just cannot persist it

v18.2 ENHANCEMENTS (User-Requested)
  Concert Lights - MUCH MORE VISIBLE
  • Base brightness increased from 0.50 to 0.75 (+50% brighter!)
  • Minimum brightness raised from 0.40 to 0.65 (never dims too much)
  • All beam layers significantly brighter (outer, mid, inner, core)
  • Hot-spot rings at beam tip made more intense and vivid
  • Floor glow MOVED UP from 95% to 88% screen height — NO MORE CUTOFF
  • Beam tip raised slightly for better overall visibility
  • Result: Vibrant concert lighting that enhances the stage atmosphere

  Settings Window Bug Fix
  • FIXED: "Save & Close" button now properly closes settings window
  • FIXED: "Cancel" button now properly closes settings window
  • Both buttons now call outer.destroy() before transitioning
  • No more ghost windows or stuck UI elements
  • Smooth fade transition to home screen after save/cancel

v18.1 ENHANCEMENTS (User-Requested Improvements)
  Homepage Visual Adjustments
  • Group photo scaled to 60% size (0.48 x 0.39 vs 0.80 x 0.65)
  • Photo positioned lower (0.95 vs 1.002) — no longer obscures subtitle
  • Concert lights brightness increased (1.0 vs 0.72) for vibrant stage feel
  • Enhanced colorful lighting creates dynamic concert atmosphere

  Aces Trivia Game Enhancements
  • Added Start/Exit confirmation modal before game begins
  • Trivia title perfectly centered in header
  • "Please wait for next question..." transition text after answer confirm
  • Improved UX flow: select → confirm → waiting feedback → next question

  Song Selection Window
  • Audio previews auto-play on carousel navigation (already implemented)
  • 700ms delay provides smooth preview experience
  • Preview stops when leaving song selection screen
  • Place preview MP3s in songs/preview/ directory

v17 Changes vs v16 / bgyo_game.py
  UI
  • All buttons use standardised colorful-rectangle style from BTN_COLORS
  • Gray sound/mute icon REMOVED from upper-right corner
  • Ghost-window artifact on menu button press FIXED (no circle BG overlap)
  • Song title + cover art in gameplay HUD centered above the track
  • Back/next arrows are large, theme-colored, consistently positioned
  • Header elements center-aligned with cover photo

  Gameplay
  • Beat chart calls audio_engine.build_beat_chart with difficulty dict
    so ival_min spacing is enforced — notes are accurate to the beat
  • Background star/confetti spawn rate halved during active gameplay
  • Hit effects always fully visible — no SFX_INTENSITY gating

  Trivia
  • Answer buttons readable: dark text on light bg, white text on dark bg
  • "Confirm Answer" button added — choice is locked only after confirm
  • "Clear All Scores" button REMOVED from leaderboard
  • Name prompt before saving trivia result to SQLite

  Leaderboard
  • Uses database.py (bgyo_data.db) exclusively — no JSON fallback
  • Difficulty filter tabs
  • No clear/delete button

  Account
  • create_account raises AccountError with specific message codes
  • UI shows exact error (USERNAME_TAKEN, PASSWORD_TOO_SHORT, etc.)
  • Password confirm field with mismatch detection

  Settings
  • Song upload/add feature REMOVED
  • Master Volume, Music Volume, SFX Volume sliders — fully functional
  • Fullscreen toggle — pixel-style button, applies immediately
  • Save & Close saves to DB and navigates away; Cancel restores values

  BGM
  • play_menu_bgm() guaranteed-different track on every home visit
"""
