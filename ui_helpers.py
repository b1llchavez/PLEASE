"""
╔══════════════════════════════════════════════════════════════════╗
║   BGYO: THE LIGHT STAGE — ACES OF P-POP  v18.2                 ║
║   ui_helpers.py  ·  Reusable UI primitives extracted from       ║
║                     bgyo_game.py                                ║
╚══════════════════════════════════════════════════════════════════╝

PURPOSE & ROLE IN THE ARCHITECTURE
────────────────────────────────────
This module consolidates every reusable UI-drawing primitive that
was previously defined — often duplicated — inside individual screen
methods in bgyo_game.py.  Centralising them here achieves three goals:

  1. Eliminate copy-paste  — _make_pixel_btn() appeared verbatim in
     _show_login(), _show_register(), _show_settings(), and more.
     It now lives here once and is called everywhere.

  2. Single source of truth — visual constants like the 4-px highlight
     strip and the "#04000C" text colour are defined once; changing
     them here updates every button in the game simultaneously.

  3. Clean separation of concerns — bgyo_game.py owns screen layout
     and game state; this module owns the drawing primitives.

Dependency chain:

    constants.py  ←  ui_helpers.py  ←  bgyo_game.py

This module imports tkinter and constants but nothing from the game
logic layer (no database, no audio_engine, no settings_state), keeping
it purely presentational.

WIDGET TYPES DEFINED HERE
──────────────────────────
  make_pixel_btn(parent, label, col, cmd, root, width, height)
      Canvas-drawn 3-layer button with animated press state.
      Used across every screen in the game.

  make_btn(parent, text, bg, fg, cmd, root, width)
      Thin wrapper around tk.Button for legacy/fallback use.

  make_themed_btn_row(parent, items, root, start_idx)
      Places a horizontal row of colorful buttons cycling BTN_COLORS.

  make_section_label(parent, txt)
      Horizontal rule with a left-aligned label — used in settings.

  draw_fancy_title(parent, bg)
      The multi-layer neon "BGYO / READY FOR THE BEAT, ACE?" title
      rendered onto a Canvas widget — shown on login/register screens.

  update_neon_border(container, t)
      Animates a cycling neon glow border on a tk.Frame using
      highlightbackground — called every frame by the main loop.

  draw_stars(cv, stars, t, w, h)
      Renders the parallax starfield background on any Canvas.

  draw_transition_overlay(cv, alpha, w, h)
      Draws the full-screen white fade rectangle during screen transitions.

  draw_star5(cv, cx, cy, r, col)
      Draws a filled 5-pointed star polygon — used for particles,
      side-panel orbiting stars, and PERFECT/combo effects.

  draw_ellipse_glow(cv, cx, cy, rx, ry, hex_col, layers, max_alpha)
      Draws concentric ellipses blending from BG_COL toward hex_col,
      creating a soft radial glow effect.

  synthesise_click_sound()
      Generates a short 880 Hz click PCM buffer using pygame.mixer.Sound.
      Called once and cached; the Sound object is reused on every click.
"""
