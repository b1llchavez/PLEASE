...
║   game_renderer.py  ·  All draw methods for the gameplay screen ║
╚══════════════════════════════════════════════════════════════════╝

PURPOSE & ROLE IN THE ARCHITECTURE
────────────────────────────────────
This module owns every Canvas drawing call that belongs to the active
gameplay screen.  It was extracted from bgyo_game.py to implement the
three-layer separation of concerns introduced in v18.2:

    game_logic.py    — WHAT happens (rules, physics, scoring)
    game_renderer.py — HOW it looks (tkinter Canvas draw calls)
    bgyo_game.py     — WHEN and WHERE (loop scheduling, screen routing)

All functions are stateless helpers that accept the Canvas, game
state, and layout parameters as arguments.  They never import
settings_state, database, or audio_engine — they only read constants
and draw.

Dependency chain:

    constants.py
    ui_helpers.py   ←  game_renderer.py  ←  bgyo_game.py

DRAW ORDER (called in sequence by bgyo_game._draw_game)
────────────────────────────────────────────────────────
  1. draw_track()          — 3D perspective lane grid + hit bar
  2. draw_game_banner()    — optional wide stage photo behind notes
  3. draw_side_effects()   — screen-edge PERFECT/COMBO/MISS labels
  4. draw_combo_panel()    — brief combo-milestone side cheer
  5. draw_notes()          — the notes the player hits
  6. draw_lane_targets()   — hit-zone markers at the hit bar
  7. draw_particles()      — per-note hit burst particles
  8. draw_sparks()         — per-PERFECT spark flares
  9. draw_flashes()        — floating feedback text ("PERFECT!", "+300")
 10. draw_countdown()      — pre-game 3…2…1…GO! display
 11. draw_combo_burst()    — ★ Nx COMBO! below the track
 12. draw_pause_overlay()  — SPACE-key pause panel
 13. draw_loading_bar()    — "⟳ Syncing beats" indicator
 14. draw_hud()            — score / combo / song title / progress bar

COORDINATE CONVENTIONS
───────────────────────
All functions accept `w` and `h` (canvas dimensions) and scale
positions relative to them, so the layout is correct in both
windowed (1080×700) and fullscreen modes.

`sx(x)` and `sy(y)` scaling helpers from bgyo_game are replicated
here as inline expressions:
    screen_x = int(x * w / BASE_W)
    screen_y = int(y * h / BASE_H)
"""
