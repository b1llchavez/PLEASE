...
║   audio_engine.py  ·  pygame audio + librosa beat detection     ║
╚══════════════════════════════════════════════════════════════════╝

PURPOSE & ROLE IN THE ARCHITECTURE
────────────────────────────────────
This module owns everything audio — both the offline beat-analysis
pipeline and the runtime audio playback player.  It sits one level
above constants.py in the dependency chain:

    constants.py  ←  audio_engine.py  ←  bgyo_game.py
                                       ←  settings_state.py (write-only via apply())

Consumers and what they use:
  bgyo_game.py       — `audio` singleton for all playback control:
                        audio.play(), audio.play_menu_bgm(),
                        audio.play_preview(), audio.stop_preview(),
                        audio.fadeout_for_game(), audio.stop(),
                        audio.pause(), audio.unpause(),
                        audio.position(), audio.is_busy()
                        analyse_beats() + build_beat_chart() for the
                        background beat-analysis worker thread.
  settings_state.py  — audio.set_volume() called by apply() on every
                        slider change; never imports audio_engine at
                        module level (imported inside apply()) to avoid
                        a circular dependency.

OPTIONAL DEPENDENCIES
──────────────────────
Both pygame and librosa are treated as optional.  The PYGAME_OK and
LIBROSA_OK flags gate every call so the game runs (without sound /
without beat sync) even if one or both libraries are absent.

  pygame   — runtime audio: BGM, previews, pause/unpause, volume.
             Install: pip install pygame
  librosa  — offline beat / onset detection for chart generation.
             Install: pip install librosa

TWO AUDIO CHANNELS
───────────────────
pygame.mixer exposes two independent channels used here:

  Music channel  (pygame.mixer.music)
      Streaming MP3 player — used for menu BGM and gameplay tracks.
      Only one track plays at a time.  Controlled by:
        music.load / play / pause / unpause / fadeout / stop / set_volume

  Sound channel  (pygame.mixer.Sound + Channel)
      In-memory buffer — used for short song-preview clips during the
      song-select screen.  The preview Sound object is stored in
      _preview_snd; its Channel handle in _preview_channel.
      Crucially, stop_preview() only stops the Sound channel —
      it NEVER calls pygame.mixer.stop() (which would kill the BGM too).

v18.0 CHANGES
──────────────
  analyse_beats()
    • beat_track tightness raised to 100 — looser snap to tempo grid,
      catches more real beats in tracks with rhythmic swing.
    • onset delta lowered to 0.07 — detects softer/off-beat transients
      that a higher threshold would miss.
    • wait frames reduced so dense passages aren't skipped.
    • 30 ms dedup window collapses near-duplicate events before the
      MIN_BEAT_GAP thinning pass — prevents double-notes.
    • MIN_BEAT_GAP is re-read from constants on every call so tuning
      changes take effect without clearing the .beats cache.

  build_beat_chart()
    • travel_time = HIT_DEPTH / speed now uses the ACTUAL difficulty
      speed at runtime — notes arrive exactly on the beat at every
      difficulty level, not only at a hard-coded reference speed.
    • ival_min enforcement uses 40% of the difficulty preset so the
      chart density stays true to the song while still being playable.
"""
